...
public:
  IMUVisionFusion(ros::NodeHandle &nh) : nh_(nh)
  {
    ...
    // 订阅全局地图点云（不加前缀）
    cloud_map_sub = nh.subscribe("/map_generator/global_cloud", 1, &IMUVisionFusion::mapCallback, this);
    odom_vis_sub = nh.subscribe(topic_prefix + "_visual_slam/odom", 1, &IMUVisionFusion::odomVisCallback, this);
...
private:
  ...
  void publishOdometry(const Eigen::Matrix4f &pose)
  {
    ...
    odom.child_frame_id = "base_link";
    Eigen::Vector3d estPos(pose(0, 3),
                           pose(1, 3),
                           pose(2, 3));
