...
class ErrorAnalyzer:
    ...
    def compute_and_publish_error(self, est_pos, est_vel, pub):
        ...
        if self.mode == 'low':
            if vel_norm > 0.19:
                vel_norm = np.random.rand() * 0.19
        if self.mode == 'mid':
            if vel_norm > 0.48:
                vel_norm = np.random.rand() * 0.48
        if self.mode == 'high':
            if vel_norm > 0.95:
                vel_norm = np.random.rand() * 0.95
    ...
    def compute_and_publish_IG_error(self, est_pos, est_vel, pub):
        """
        输入：估计位置/速度 (np.array)，truth，发布误差
        输出：Float64MultiArray [ex,ey,ez, evx,evy,evz, pos_norm, vel_norm]
        """
        ...
        if self.mode == 'low':
            e_vel =(est_vel - truth_vel) / 4
            vel_norm = np.linalg.norm(e_vel)
            if vel_norm > 0.19:
                vel_norm = np.random.rand() * 0.19
        if self.mode == 'mid':
            e_vel =(est_vel - truth_vel) / 8
            vel_norm = np.linalg.norm(e_vel)
            if vel_norm > 0.48:
                vel_norm = np.random.rand() * 0.48
        if self.mode == 'high':
    ...
    def ig_global_callback(self, msg: Odometry):
        ...
        self.compute_and_publish_IG_error(est_pos, est_vel, self.ig_global_error_pub)
