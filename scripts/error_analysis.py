...
class ErrorAnalyzer:
    def __init__(self):
        ...
        # self.iv_hp_sub = rospy.Subscriber(
        #     f'{drone_prefix}/odom_fused_IV_hp', Odometry, self.iv_hp_callback
        # )
        ...
        # self.iv_hp_error_pub = rospy.Publisher(
        #     f'{drone_prefix}/error_iv_hp', Float64MultiArray, queue_size=10
        # )
    ...
    def compute_and_publish_error(self, est_pos, est_vel, pub):
        ...
        # --- 范数 ---
        norm_truth_pos = np.linalg.norm(truth_pos)
        norm_est_pos   = np.linalg.norm(est_pos)
        pos_norm = abs(norm_est_pos - norm_truth_pos)

        norm_truth_vel = np.linalg.norm(truth_vel)
        norm_est_vel   = np.linalg.norm(est_vel)
        vel_norm = abs(norm_est_vel - norm_truth_vel)

        # --- 填充消息 ---
        msg_out = Float64MultiArray()
        msg_out.data = [
            e_pos[0], e_pos[1], e_pos[2], pos_norm,
            e_vel[0], e_vel[1], e_vel[2], vel_norm,
            norm_truth_vel, norm_est_vel
        ]
